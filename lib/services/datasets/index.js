...
import { getChunkById, getChunkByProjectId } from '@/lib/db/chunks';
...
export async function generateDatasetForQuestion (projectId, questionId, options) {
  try {
    ...
    let chunkList = await getChunkByProjectId(projectId)
    let contentStr = ''
    for (let chunkItem of chunkList) {
      if (!chunkItem) {
        throw new Error('文本块不存在');
      }
      if ((contentStr.length + chunkItem.content.length) < 30000) {
        contentStr += chunkItem.content
      }
    }
    // 获取该文本块内容--配置
    ...
    let idDistill = chunk.name === 'Distilled Content';
