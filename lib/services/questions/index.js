...
import getProblemToKeywords from '@/lib/llm/prompts/problemToKeywords';
...
export async function generateQuestionsForChunk (projectId, chunkId, options) {
  try {
    ...
    // 生成文本块关键词
    const keywordsPrompt = getProblemToKeywords({
      text: chunk.content,
      globalPrompt,
      questionPrompt
    });
    const keywordsRes = await llmClient.getResponse(keywordsPrompt);
    const keywords = keywordsRes.split('</think>')[1]
...
export async function generateQuestionsForChunkWithGA (projectId, chunkId, options) {
  try {
    ...
    // 计算基础问题数量
    const baseQuestionNumber = number || questionGenerationLength;
    ...
    if (useGaExpansion) {
      ...
      const labelPrompt = labelPromptFunc(JSON.stringify(tags), JSON.stringify(questions));
      const labelResponse = await llmClient.getResponse(labelPrompt);
      const labelRes = labelResponse.split('</think>')[1]
      console.log('%c [ labelResponse ]-275', 'font-size:13px; background:pink; color:#bf2c9f;', labelResponse)
      const labelQuestions = extractJsonFromLLMOutput(labelRes);
