    ...
    try {
      ...
      if (exportOptions.asyncFlow) {
        ...
        fetch(url, {
          ...
          .then(data => {
            console.log('%c [ data.data[0].id ]-761', 'font-size:13px; background:pink; color:#bf2c9f;', data.data[0].id)
      ...
      .then(response => {
        if (!response.ok) {
          setConfirmDialog({
            open: true,
            title: '错误提示',
            content: '数据集解析失败' + response.statusText,
          });
        }
        return response.json()
      })
