  ...
  const handleExportDatasets = exportOptions => {
    try {
      ...
      if (exportOptions.asyncFlow) {
        ...
        console.log('%c [ exportOptions.fileName ]-740', 'font-size:13px; background:pink; color:#bf2c9f;', exportOptions.fileName)
        formData.append('file', blob, exportOptions.fileName + '.txt')
        fetch(url, {
          method: 'POST',
          headers: config.ragflow.headers,
          body: formData
        })
          .then(res => {
            console.log('%c [ res ]-745', 'font-size:13px; background:pink; color:#bf2c9f;', res.json())
          })
          .then(data => {
            console.log('%c [ data ]-751', 'font-size:13px; background:pink; color:#bf2c9f;', data)
          })
        // parseChunks(res.json().data[0].id)  
        // fetch(`${config.ragflow.base_url}/api/v1/datasets/${config.ragflow.datasets.qa_library.dataset_id}/documents`)
